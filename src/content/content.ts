...
let facebookBannerEnabled = true;
let instagramSidebarEnabled = true;
...
function ensureStyle(styleId: string, css: string) {
  ...
  if (existing) {
    // Keep CSS synced across hot reloads / extension reloads without requiring stale nodes to be removed first.
...
function applyInstagramSidebarState() {
  ...
  if (instagramSidebarEnabled) {
    document.documentElement.classList.add(INSTAGRAM_SIDEBAR_CLASS);
    ensureStyle(
      INSTAGRAM_SIDEBAR_STYLE_ID,
      [
        // Only hide Instagram's global left navigation, not the Direct conversation list.
        ...
        // Explicit desktop navigation class signature reported by user.
        ...
        // Explicit mobile navigation class signature reported by user.
...
chrome.runtime.sendMessage({ type: 'GET_BANNER_STATE' }, (resp: { enabled?: boolean } | undefined) => {
  if (typeof resp?.enabled === 'boolean') {
    facebookBannerEnabled = resp.enabled;
    applyFacebookBannerState();
  }
});
...
chrome.runtime.sendMessage({ type: 'GET_INSTAGRAM_SIDEBAR_STATE' }, (resp: { enabled?: boolean } | undefined) => {
  if (typeof resp?.enabled === 'boolean') {
    instagramSidebarEnabled = resp.enabled;
    applyInstagramSidebarState();
  }
});
...
chrome.runtime.onMessage.addListener((msg: any) => {
  if (msg.type === 'BANNER_STATE_CHANGED') {
    if (typeof msg.enabled === 'boolean') {
      facebookBannerEnabled = msg.enabled;
      applyFacebookBannerState();
    }
  }

  if (msg.type === 'INSTAGRAM_SIDEBAR_STATE_CHANGED') {
    if (typeof msg.enabled === 'boolean') {
      instagramSidebarEnabled = msg.enabled;
      applyInstagramSidebarState();
    }
  }
});
